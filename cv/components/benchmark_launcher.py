...
from aux.common import wait_for_launches, NestedLoop, kill_launches, clear_symlink, update_symlink
from components import DEFAULT_VERIFIER_TOOL, BUSY_WAITING_INTERVAL, DefaultDir, CloudInfo
...
from models.verification_result import VerificationResults, Resource, TERMINATION_SUCCESS
...
class BenchmarkLauncher(Launcher):
    ...
    def __init__(self, config_file, additional_config: dict, is_launch=False):
        super().__init__(ComponentName.BENCHMARK_LAUNCHER, config_file)
    ...
    def __parse_result_file(self, file: str, group_directory: str):
        ...
        config = {}
        if memory_limit:
            config[Tag.CONFIG_MEMORY_LIMIT] = memory_limit
        if time_limit:
            config[Tag.CONFIG_CPU_TIME_LIMIT] = time_limit
        if cpu_cores_limit:
            config[Tag.CONFIG_CPU_CORES_LIMIT] = cpu_cores_limit
        if options:
            config[Tag.CONFIG_OPTIONS] = options
    ...
    def _export_results(self, results: list, config: dict, add_res: dict = None) -> str:
        ...
        coverage_resources = {Resource.CPU_TIME: 0.0, Resource.WALL_TIME: 0.0, Resource.MEMORY_USAGE: 0}
        mea_resources = {Resource.CPU_TIME: 0.0, Resource.WALL_TIME: 0.0, Resource.MEMORY_USAGE: 0}
        for result in results:
            coverage_resources[Resource.MEMORY_USAGE] = \
                max(coverage_resources[Resource.MEMORY_USAGE],
                    result.coverage_resources.get(Resource.MEMORY_USAGE, 0))
            mea_resources[Resource.MEMORY_USAGE] = max(mea_resources[Resource.MEMORY_USAGE],
                                                       result.mea_resources.get(Resource.MEMORY_USAGE, 0))
            coverage_resources[Resource.CPU_TIME] += result.coverage_resources.get(Resource.CPU_TIME, 0.0)
            coverage_resources[Resource.WALL_TIME] += result.coverage_resources.get(Resource.WALL_TIME, 0.0)
            mea_resources[Resource.CPU_TIME] += result.mea_resources.get(Resource.CPU_TIME, 0.0)
            mea_resources[Resource.WALL_TIME] += result.mea_resources.get(Resource.WALL_TIME, 0.0)
        ...
        with open(report_components, "w", encoding='ascii') as f_report:
            ...
            f_report.write(f"{ComponentName.BENCHMARK_LAUNCHER};"
            ...
            f_report.write(f"{ComponentName.MEA};"
                           f"{round(mea_resources[Resource.CPU_TIME], ROUND_DIGITS)};"
                           f"{round(mea_resources[Resource.WALL_TIME], ROUND_DIGITS)};"
                           f"{mea_resources[Resource.MEMORY_USAGE]}\n")
            f_report.write(f"{ComponentName.COVERAGE};"
                           f"{round(coverage_resources[Resource.CPU_TIME], ROUND_DIGITS)};"
                           f"{round(coverage_resources[Resource.WALL_TIME], ROUND_DIGITS)};"
                           f"{coverage_resources[Resource.MEMORY_USAGE]}\n")
            if add_res:
                for name, res in add_res.items():
                    f_report.write(f"{name};"
                                   f"{round(res[Resource.CPU_TIME], ROUND_DIGITS)};"
                                   f"{round(res[Resource.WALL_TIME], ROUND_DIGITS)};"
                                   f"{res[Resource.MEMORY_USAGE]}\n")
    ...
    def __upload(self, report_launches, report_resources, report_components, result_archive,
                 config):
        exporter = Exporter(self.config, DefaultDir.EXPORT, self.install_dir, tool=self.tool)
    ...
    def launch_benchmark(self):
        ...
        Launch benchmark.
        """
        if not self.scheduler or not self.scheduler == CloudInfo.SCHEDULER_CLOUD:
        ...
        log_file_name = os.path.join(self.output_dir, CloudInfo.CLOUD_BENCHMARK_LOG)
    ...
    def __process_witnesses_only(self, uploader_config, is_upload):
        ...
        self.config[ComponentName.MEA][Tag.SOURCE_DIR] = self.tasks_dir
