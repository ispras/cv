...
from components import CoverageInfo, ERROR_TRACE_SOURCES, DEFAULT_VERIFIER_TOOL
...
from models.verification_result import WitnessType, PropertiesDescription, Resource, Verdict, PROPERTY_IS_RELEVANCE, \
    PROPERTY_IS_ALL_TRACES_FOUND, TERMINATION_SUCCESS, ADDITIONAL_RESOURCES, Property
...
class Exporter(Component):
    ...
    def __init__(self, config, work_dir: str, install_dir: str,
        ...
        super().__init__(ComponentName.EXPORTER, config)
        self.work_dir = work_dir
        self.install_dir = install_dir
        self.version = self.component_config.get(Tag.VERSION)
        self.add_logs = self.component_config.get(Tag.ADD_VERIFIER_LOGS, True)
        self.add_proofs = self.component_config.get(Tag.ADD_VERIFIER_PROOFS, True)
    ...
    def __process_coverage(final_zip: zipfile.ZipFile, verifier_counter: int, work_dir: str,
        ...
        cov_name = None
        coverage = os.path.join(os.path.join(work_dir, CoverageInfo.DEFAULT_COVERAGE_ARCH))
        if os.path.exists(coverage):
            if not ignore:
                cov_name = f"coverage_{verifier_counter}.zip"
                final_zip.write(coverage, arcname=cov_name)

            coverage_src = os.path.join(os.path.join(work_dir, CoverageInfo.DEFAULT_COVERAGE_SOURCE_FILES))
    ...
    def __process_specific_coverage(self, work_dirs: list, cov_type: str,
        ...
        for work_dir in work_dirs:
            ...
            if os.path.exists(coverage):
                with zipfile.ZipFile(coverage) as tmp_arch:
                    data = json.loads(tmp_arch.read(CoverageInfo.DEFAULT_COVERAGE_FILE).
        ...
        if is_rule:
            for merge_type, results in coverage_by_rule.items():
                if not results[Tag.STATISTICS]:
                    results[Tag.STATISTICS] = stats
                merge_coverages(function_coverage, results[Tag.FUNCTION_COVERAGE],
                                line_coverage, results[Tag.LINE_COVERAGE], merge_type)
    ...
    def __process_global_coverage(self, global_cov_files: dict, final_zip: zipfile.ZipFile):
        counter = 0
        coverage_by_rule = {
            CoverageInfo.COVERAGE_MERGE_TYPE_UNION: {},
            CoverageInfo.COVERAGE_MERGE_TYPE_INTERSECTION: {}
        }
        for _, description in coverage_by_rule.items():
            description[Tag.FUNCTION_COVERAGE] = {}
            description[Tag.LINE_COVERAGE] = {}
        ...
        for cov_type, work_dirs in global_cov_files.items():
            ...
            if cov_type == GLOBAL_COVERAGE_REAL:
                ...
                counter = self.__process_specific_coverage(work_dirs, cov_type, final_zip, counter,
                                                           coverage_by_rule)
        for merge_type, results in coverage_by_rule.items():
            self.__print_coverage(final_zip, counter, results[Tag.FUNCTION_COVERAGE],
                                  results[Tag.LINE_COVERAGE], results[Tag.STATISTICS], merge_type)
            counter += 1
    ...
    def export(self, report_launches: str, report_resources: str, report_components: str,
        ...
        if self.version:
            root_element['attrs'] = [self.__format_attr(Tag.VERSION, self.version)]
        else:
            root_element['attrs'] = []
                    ...
                    if res:
                        ...
                        if unknown_desc and name in unknown_desc:
                            ...
                            for u_desc in unknown_desc[name]:
                                ...
                                unknown_report['attrs'] = u_desc.get(Tag.ATTRS)
                                unknown_report['resources'] = {
                                    "CPU time": u_desc[Resource.CPU_TIME],
                                    "memory size": u_desc[Resource.MEMORY_USAGE],
                                    "wall time": u_desc[Resource.WALL_TIME]
                                }
                                reports.append(unknown_report)
                        if component_attrs and name in component_attrs:
                            new_report['attrs'] = component_attrs[name]
                        reports.append(new_report)
                        if name == ComponentName.LAUNCHER:
                            launcher_id += name
                            overall_wall += wall
                        overall_cpu += cpu
            ...
            with open(report_launches, encoding='utf8', errors='ignore') as file_obj, \
                ...
                for line in file_obj.readlines():
                    ...
                    if res:
                        ...
                        if rule == Property.COVERAGE:
                            ...
                            if not if_coverage_sources_written:
                                verification_element['coverage sources'] = \
                                    CoverageInfo.DEFAULT_COVERAGE_SOURCES_ARCH
                        ...
                        reports.append(verification_element)
                        witnesses = glob.glob(
                            f"{work_dir}/{WitnessType.VIOLATION}_witness*{Extension.ARCHIVE}")
                        ...
                        if not witnesses or incomplete_result:
                            other_element = {}
                            witnesses = glob.glob(
                                f"{work_dir}/{WitnessType.CORRECTNESS}_witness*{Extension.ARCHIVE}")
                ...
                for report in reports:
                    if report.get("type") == "component" and report.get("name") == ComponentName.MEA:
                ...
                reports.append(self.__create_component_report(ComponentName.EXPORTER, self.cpu_time,
                ...
                with zipfile.ZipFile(CoverageInfo.DEFAULT_COVERAGE_SOURCES_ARCH, mode='w',
                                     compression=zipfile.ZIP_DEFLATED) as arch_obj:
                    src_paths = set()
                ...
                final_zip.write(CoverageInfo.DEFAULT_COVERAGE_SOURCES_ARCH)
                os.remove(CoverageInfo.DEFAULT_COVERAGE_SOURCES_ARCH)
