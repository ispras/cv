...
from components import LOG_FILE, DefaultDir, CoverageInfo
from components.component import Component
from coverage.lcov import LCOV
from models.tags import Tag, ComponentName
...
def extract_internal_coverage(data: dict, function_coverage: dict, line_coverage: dict,
    ...
    """
    __extract_internal_coverage(data.get(Tag.FUNCTION_COVERAGE, {}).get(Tag.COVERAGE, []),
                                function_coverage)
    __extract_internal_coverage(data.get(Tag.LINE_COVERAGE, []), line_coverage)
    for file, funcs in data.get(Tag.FUNCTIONS_STATISTICS, {}).get(Tag.STATISTICS, {}).items():
...
def __merge_coverages(input_data: dict, output_data: dict, merge_type: str):
    for encoded_elem, covered_num in input_data.items():
        if merge_type == CoverageInfo.COVERAGE_MERGE_TYPE_UNION:
            output_data[encoded_elem] = max(output_data.get(encoded_elem, 0), covered_num)
        elif merge_type == CoverageInfo.COVERAGE_MERGE_TYPE_INTERSECTION:
            if encoded_elem not in output_data:
...
def write_coverage(counter: int, function_coverage: dict, line_coverage: dict, stats: dict) -> str:
    ...
    data = {
        Tag.FUNCTION_COVERAGE: {
            Tag.COVERAGE: [],
            Tag.STATISTICS: {}
        },
        Tag.LINE_COVERAGE: [],
        Tag.FUNCTIONS_STATISTICS: {
            Tag.STATISTICS: stats,
            Tag.VALUES: []
        },
        TAG_PERCENT: {}
    }
    __decode_coverage(function_coverage, data[Tag.FUNCTION_COVERAGE][Tag.COVERAGE])
    __decode_coverage(line_coverage, data[Tag.LINE_COVERAGE])
    functions_percent, lines_percent = _count_percent(function_coverage, line_coverage)
    data[TAG_PERCENT][Tag.FUNCTION_COVERAGE] = functions_percent
    data[TAG_PERCENT][Tag.LINE_COVERAGE] = lines_percent
    data[TAG_PERCENT][Tag.STATISTICS] = {}
    data[TAG_PERCENT][Tag.VALUES] = []
    with zipfile.ZipFile(generated_arch, mode='w', compression=zipfile.ZIP_DEFLATED) as arch_obj:
        ...
        arch_obj.write(generated_cov, arcname=CoverageInfo.DEFAULT_COVERAGE_FILE)
...
def __count_percent(elements: dict) -> tuple:
    ...
    for encoded_elem, covered_num in elements.items():
        if str(encoded_elem).startswith(Tag.SOURCES):
            counter += 1
            if covered_num > 0:
                covered += 1
...
class Coverage(Component):
    ...
    def __init__(self, launcher_component: Component = None, basic_config=None, install_dir=None,
        ...
        if launcher_component:
            ...
            config = basic_config
        super().__init__(ComponentName.COVERAGE, config)
        if launcher_component:
        ...
        self.internal_logger = logging.getLogger(name=ComponentName.COVERAGE)
    ...
    def compute_coverage(self, source_dirs: set, launch_directory: str,
        ...
        if queue:
            ...
            data[Tag.COVERAGE_LINES] = cov_lines
            data[Tag.COVERAGE_FUNCS] = cov_funcs
            queue.put(data)

    def __full_coverage(self, source_dirs: set, coverage_file: str, work_dir: str):
        dummy_dir = ""
        for src_dir in source_dirs:
            if os.path.exists(os.path.join(src_dir, DefaultDir.CLADE)):
                dummy_dir = os.path.join(src_dir, DefaultDir.CLADE)
                break
        ...
        archive = os.path.join(CoverageInfo.DEFAULT_COVERAGE_ARCH)
        files = [CoverageInfo.DEFAULT_COVERAGE_FILE] + list(lcov.arcnames.keys())
        with open(archive, mode='w+b', buffering=0) as arch_obj:
            with zipfile.ZipFile(arch_obj, mode='w', compression=zipfile.ZIP_DEFLATED) as extr_obj:
                with open(CoverageInfo.DEFAULT_COVERAGE_SOURCE_FILES, mode="w", encoding='utf8') as file_obj:
                    for file in files:
                        arch_name = lcov.arcnames.get(file, os.path.basename(file))
                        if file == CoverageInfo.DEFAULT_COVERAGE_FILE:
