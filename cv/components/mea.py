...
class MEA(Component):
    ...
    def __init__(self, general_config: dict, error_traces: list, install_dir: str, rule: str = "",
        ...
        super().__init__(ComponentName.MEA, general_config)
    ...
    def __print_parsed_error_trace(self, parsed_error_trace: dict, converted_error_trace: list,
        ...
        converted_traces = {}
        if parsed_error_trace.get('type') == WitnessType.VIOLATION:
            for conversion_function in CACHED_CONVERSION_FUNCTIONS:
    ...
    def __get_aux_file_names(error_trace_file: str) -> tuple:
        ...
        common_part = error_trace_file[:-len(Extension.GRAPHML)] + "_"
        json_trace_name = common_part + Extension.JSON
    ...
    def __export_et_parser_lib(self):
        et_html_lib = self.get_tool_path(self._get_tool_default_path(Tool.ET_HTML_LIB),
                                         self.config.get(Tag.TOOLS, {}).get(Tool.ET_HTML_LIB))
    ...
    def clear(self):
        ...
        if self.error_traces:
            ...
            for file in glob.glob(os.path.join(work_dir, f"*{Extension.JSON}")):
                os.remove(file)
