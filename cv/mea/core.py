...
class ConversionFunction(str, Enum):
    """
    MEA conversion functions
    """
    CALL_TREE = "call tree"
    MODEL_FUNCTIONS = "model functions"
    CONDITIONS = "conditions"
    ASSIGNMENTS = "assignments"
    NOTES = "error descriptions"
    FULL = "full"


DEFAULT_CONVERSION_FUNCTION = ConversionFunction.MODEL_FUNCTIONS
CACHED_CONVERSION_FUNCTIONS = [
    ConversionFunction.CALL_TREE,
    ConversionFunction.MODEL_FUNCTIONS,
    ConversionFunction.NOTES]


# Comparison functions.
class ComparisonFunction(str, Enum):
    """
    MEA comparison functions.
    """
    EQUAL = "equal"
    INCLUDE = "include"
    INCLUDE_WITH_ERROR = "include with error"
    INCLUDE_PARTIAL = "partial include"
    INCLUDE_PARTIAL_ORDERED = "partial include ordered"
    SKIP = "skip"


DEFAULT_COMPARISON_FUNCTION = ComparisonFunction.EQUAL
...
# Conversion functions arguments.
...
def convert_error_trace(error_trace: dict, conversion_function: str, args: dict = dict) -> list:
    ...
    """
    functions = {
        ConversionFunction.MODEL_FUNCTIONS: __convert_model_functions,
        ConversionFunction.CALL_TREE: __convert_call_tree_filter,
        ConversionFunction.CONDITIONS: __convert_conditions,
        ConversionFunction.FULL: __convert_full,
        ConversionFunction.ASSIGNMENTS: __convert_assignments,
        ConversionFunction.NOTES: __convert_notes
    }
    ...
    if (args.get(TAG_USE_NOTES, args.get(TAG_USE_WARNS, False)) or
            ...
            conversion_function not in [ConversionFunction.FULL, ConversionFunction.NOTES]:
...
def compare_error_traces(edited_error_trace: list, compared_error_trace: list,
    ...
    if not et1_threaded and not et2_threaded:
        ...
        return 1.0
    functions = {
        ComparisonFunction.EQUAL: __compare_equal,
        ComparisonFunction.INCLUDE: __compare_include,
        ComparisonFunction.INCLUDE_WITH_ERROR: __compare_include_with_error,
        ComparisonFunction.INCLUDE_PARTIAL: __compare_include_partial,
        ComparisonFunction.INCLUDE_PARTIAL_ORDERED: __compare_include_partial_ordered,
        ComparisonFunction.SKIP: __compare_skip
    }
