Инструментарий для ведения непрерывной верификации CV
-----------------------------------------------------
...
Конфигурационные файлы плагина включают следующие элементы:
- `entrypoints` (описание генерируемых точек входа для верифицируемой системы) — добавляются файлы, описывающие множество точек входа и проверяемую подсистему (по шаблону);
- `properties/properties.json` (описание дополнительных правил корректности);
- `properties/automata` (спецификации правил корреткности в виде автомата);
- `properties/models` (спецификации правил корреткности на языке C, которые будут добавлены в процесс сборки  (в данном файле могут быть определены дополнительные проверки или переопределена семантика существующих функций);
...
При этом верификацию можно вести относительно либо встроенного правила корректности
...
4. Пример конфигурации для верификации

* Директория с исходниками теста `docs/examples/sources/`. Сборка выполняется командой `make`.
* Описание точек входа `entrypoints/it.json`.
* Описание правила корректности проверки использования памяти `properties/properties.json`.
* Конфигурация запуска `configs/it.json`.
Для запуска необходимо выполнить в рабочей директории:
```shell
